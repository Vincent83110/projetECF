// Gestion de l'espace employé avec recherche et affichage des avis

// Gestion du formulaire de recherche d'employés
  ...
  if (pseudo && numero) {
    ...
    fetch(`${BASE_URL}/actions/getStatut.php?username=${encodeURIComponent(pseudo)}`)
      ...
      .then(data => {
        ...
        if (!data.success) {
          alert("Utilisateur introuvableeeee.");
          return;
        }
...
window.addEventListener('load', () => {
  ...
  avisContainers.forEach(container => {
    ...
    // Vérification après un délai pour s'assurer du rendu complet
    setTimeout(() => {
    ...
    // Basculer entre voir plus et voir moins
